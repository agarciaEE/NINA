...
You can summarize the output by using \code{summary()} function or \code{print()}:
```{r summary}
summary(g1_EN)
print(g2_BC)
```

You can also plot a summary of the output model using \code{plot()} function:

```{r plot, echo = FALSE}
plot(g2_BC)
```

Models can be evaluated using NINA's function \code{models_evaluation()} and visualize it by plotting the output.

```{r eval}
eval <- models_evaluation(g2_EC, plot = F)
plot(eval)
````
