...
for (i in 1:n_sp1){
  ...
  sp1.l[[i]] <- sp$probability.of.occurrence
...
for (i in 1:n_sp2){
  ...
  biomask <- sum(stack(sp1.l[Xvar]))
  biomask[biomask > minValue(biomask)] = 1
  biomask[biomask < 1] = NA
  sp <- generateRandomSp(mask(env_data, biomask), niche.breadth = sample(c("wide", "narrow"), 1))
  occ = sampleOccurrences(sp, n = sample(10:100, 1))
...
rm(n_sp1, n_sp2, sp, sp1.l, biomask, Xvar, i, occ)
